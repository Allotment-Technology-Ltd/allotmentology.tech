...
- **Runtime**: `await createFundingOpsAiContext({ userId, opportunityId })` from `src/lib/ai/runtime.ts` (throws if AI is not configured). Per-user OpenAI-compatible providers from **Settings → BYOK & AI keys** (multiple keys, default + revoke) override env when present.
